...
1. [Download the latest Unity Uploader](https://altvr.com/download-latest-unity-uploader).
    ...
    The Uploader is a set of files, contained in a .tgz file, that you'll import into Unity. These files give you menu items and a window that provides the options you need to get your Unity scene into AltspaceVR.
...
1. Navigate to the folder that you downloaded the Altspace Uploader (in other words, the .tgz file) to and then copy or move it from that folder to the root folder of your new Unity project.
...
1. Navigate to the folder that contains the .tgz file, then select it, and then click **Open**.  
    ...
    ![Screenshot of Uploader in the Package Manager](images/019-uploader-in-package-manager.png)
    ...
    > [!NOTE]
    > You'll need to import the AltspaceVR Uploader package into every Unity project you want to use with Altspace.

## Build and upload your Unity scene

1. On the menu bar, select **AltspaceVR > Templates**.
1. In the **Altspace VR Templates** window, sign in with your Altspace account credentials or MSA. If you've only ever logged into Altspace with your Microsoft account, you'll need to create a password using the "Forgot your password" option on the website.
...
1. In the **Altspace VR Templates** window, click the **Choose a .unity file** ellipsis button (three dots), then navigate to the **Assets** > **Scenes** folder in your project, and then select **Hello World Scene.unity** and open it.
...
1. Navigate to **Hello World** and then enter it.

    Your scene should be similar to what you saw in the Unity Editor. You may have to scroll around a bit to see the whole scene.
...
To see many scenes that were built using the Uploader, go to See [Featured Worlds](https://account.altvr.com/worlds/featured).
