...
| Term | Definition |
|---|---|
| World Building | AltspaceVR offers the ability to create and customize virtual worlds. AltspaceVR hosts support documentation, Discord channels, and in-world events to help you learn more about how to get help and start building worlds. |
...
| Universe | A universe in AltspaceVR world building vernacular represents the categorization of your worlds. Each universe may hold multiple worlds. The worlds inherit the settings of the universe, making it easy to add people to the allow-list for multiple worlds and other control features. Please see Managing Your Worlds for more information. |
...
| Event space | An event space is a synonym for world in AltspaceVR. In general, it refers to a world used to host events.
 |
| Website | References to the "website" are to AltspaceVR's website. It is often easier to create and edit events through the Events web page on a computer or tablet rather than through your VR device. You will need to access the website for world building as well. |
| Contextual roles | Contextual Roles are assigned by the event creator or world builder to users in a world or event space adding features and capabilities. Currently, these consist of Host, Presenter, Moderator, Pilot (flight), Terraformer (world building), and Megaphone Only. These may be assigned individually or globally, allowing everyone to have the same roles in the event space or world. |
...
| SDK/MRE | These are world building terms associated with the Software Development Kit and Mixed Reality Extensions used to add features and functionality to the world building experience. These are typically for more advanced users. |

## Creating Your Event and Event Space or World

AltspaceVR makes it easy to create a world in which you may use a pre-made template space to use or customize for your specific needs, or create a world from scratch. This article will cover using a pre-made template to create your event. The sections below on Branding Your Event with Images and Adding 3D Objects using World Editor/Kits include tips for further customization. For information on building a custom world from scratch, attend the World Building Tour meetups in AltspaceVR and check out the World Building support documentation.
...
The process of creating an event is the same for both, with one exception: the creation of a world and importing it as a repeat-use event space.
...
The following are step-by-step instructions for creating an event for a one-time or repeated event. By selecting the template or importing a world for the event. NOTE: The AltspaceVR events web page offers instructions on every aspect of the form through the use of green question mark buttons. Roll your cursor over them for specific instructions.

On the Events > My Events web page on AltspaceVR's website, click Schedule an Event or go straight to the Create Event in AltspaceVR web page.

Event Title: Type in the name of the event. Keep it specific and concise for viewing on the various calendar views in AltspaceVR's website and in-world interface. Try to keep your title under 23 characters including spaces between words.
Description: Type in the description of the event.
...
Start Date / End Date: Set the start time and make sure the end time is after the start time.
Category: Choose the category that best describes your event type. 
Set the event to Private or Public.
Visibility: Public events are visible on the AltspaceVR website event calendar's All tab and are open to the public.
...
Select a Template: Along the right side of the web page is a list of thumbnail images of the available templates in AltspaceVR. There are game rooms, offices, meeting spaces, presentation spaces, and fun meetup spaces. Select one that looks interesting. If you don't like it, go ahead and just create a new event with a new Template. If you wish to create your own custom event world, select the Cool Sky as your default template and following the instructions below for importing your world.
Click ADVANCED OPTIONS.
Promote: Each event requires 2 branding images (these images appear on the AltspaceVR website and do not appear in your event inside AltspaceVR):
...
In VR: This section includes features that apply to the virtual experience during the event inside the AltspaceVR app:
...
Delete the event.
Group: Choose from your private Groups using the drop down menu. Will only display if your account has created/been added to a Group.
...
Click the dropdown arrow to see a list of worlds you created.
Select the world.
After completing the rest of the event form, please wait at least 2 minutes before visiting the event space in AltspaceVR to ensure database information is updated and the world is generated.
...
Twitter Handle: This adds your Twitter stream to the event's web page. Be aware that if the Twitter account is personal and not related to the event or association, you may be over-sharing. You'll only need the handle: @elonmusk
Contextual Roles: This is where you control the super powers or contextual roles of your event hosts, presenters, moderators and other roles within the event. To add them, enter in their AltspaceVR username and assign them a role in the drop down menu. To remove them, click the checkbox to Remove and save the event web page. NOTE: If you wish to grant a presenter moderation capabilities, you must add them as a presenter and moderator. The following Contextual Roles are currently available:
Presenter: Adds the Front Row features to the individual's interface in AltspaceVR to add those features.
...
Block Listed Users: If you wish to block a user from access, or a user has been previously removed from the event by a moderator or presenter and the event duplicated, the block listed user's name will be listed. Event hosts or Admins may add or remove a block listed user at any time.
...
To make changes to the event settings, click EDIT.
To end the event (if your event is over early for example), click END EVENT.
To set your event to Draft instead of Active, click SET TO DRAFT.
If you'd made an update to your imported world, you'll need to click RE-IMPORT WORLD for those changes to appear in your event, don't forget to Reset Space in the event :)
To duplicate the event for future events, click DUPLICATE EVENT. This duplicate will then appear in My Events/My Draft Events, you'll need to update the new day/time and Activate it from there.
To set your event onto the calendar listing, click ADD AS MAIN EVENT.
If you wish to completely delete the event (you will not be able to recover this), click DELETE EVENT.
...
For more information on customizing your event space or creating an original event space world to import, see:

How do I grant roles to other people in my Worlds?
How do I let people fly (or have other abilities) in my World?
Where can I get help with world-building?
How do I manage my Worlds?
World-Building FAQ
How do I add custom spawn points to my Worlds?
How do I upload my own Kits?
How do I get started with the World Building Toolkit (Unity Uploader)?
Using the Multimedia Console for a slide presentation
The Multimedia Console is a great way to organize your content for a presentation including images, audio or video. Follow the instructions here to get that setup for your event: altvr.com/slides
...
The process involves uploading your image to your AltspaceVR Photos, then using the Photos in the World Editor in-world to import and place them into the event world space.

Go to Photos on the AltspaceVR website.
Click Upload.
Click Browse to open a dialog box on your computer and locate and select the optimized image.
Click Open.
Click CREATE PHOTO.
Repeat this for additional images.
...
On the user interface, in the lower right corner, click World Editor/Editor Panel
Click the Mine tab at the bottom.
Click Photos.
Photos are stored in the chronological order of upload, so the latest image uploaded should be the first image in the list. Click it to add it into the world.
Using your device cursor, grab the image and place it where you wish it to be.
...
Note that when Edit Mode is on (orange text), you will automatically have the ability to fly, allowing easier movement for placing images higher in the event world.
Repeat this for each image placement.
When the images are set in place, click Lock All if you didn't lock them individually, and turn off Edit Mode and close the editor.
Please only use a maximum of 5 Photos per event.
Adding 3D Objects Using World Editor/Kits
AltspaceVR allows importing 3D objects into worlds with the World Editor. There are many World Editor Kits currently available for use, and more on the way. There are spaceships, rockets, animated animals, sculptures, trees, plants, and many different objects. Or you may add your own Kits to the public collection or for your own personal usage, or consider using one of the AltspaceVR MRE apps, which bring interactable objects into your world such as helmets, swords, wings, clothing, hats, or other interactable objects. Expect the list of objects to expand soon.

The instructions here focus on using items already in the Kits. For more information on adding and importing 3D objects into AltspaceVR, see "How to import glTF models into my Worlds?" and "How do I upload my own Kits?"
...
Explore the kits for the object you wish to import and select it.
Click on the kit object with your cursor and position it. Use the GEAR icon on the object in the World Editor to use the Position, Rotate, and Scale options for manual number entry.
When the object is placed in the right position, lock it by clicking the Lock icon.
Add another object if you wish. Please be judicious in adding 3D objects, keeping your selections to no more than items from 5 different individual kits, for better Oculus Quest user support. Please do not use more than 200 objects in a world.
When done, lock all items, turn off Edit Mode, and close the World Editor. Wander around. Invite a few testers to visit and check it out, and get ready for your great event!
For more information on the World Editor Kits and adding 3D objects to AltspaceVR:

What is the Early Access Program?
How to import glTF models into my Worlds?
How do I use an MRE in my World?
How to Record or Live Stream My Event
